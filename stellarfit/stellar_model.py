...
class StellarModel:
    """Primary StellarFit class. Interpolates a model of an inhommogeneous
    stellar surface given a set of input parameters and a stellar model grid.
    """
    ...
    def __init__(self, input_parameters, stellar_grid, dt_min=100):
        ...
        # Make sure a reasonable spot temperature and covering fraction are
        # passed if spots are being used.
        if self.spots is True:
            for param in ['dt_spot', 'f_spot']:
                assert param in self.input_parameters.keys()
                if self.input_parameters['dt_spot']['value'] < dt_min:
                    msg = 'dt_spot less than minimum temperature contrast ' \
                          'of {}K.'.format(dt_min)
        ...
        if self.faculae is True:
            for param in ['dt_fac', 'f_fac']:
                ...
                if self.input_parameters['dt_fac']['value'] < dt_min:
                    msg = 'dt_fac less than minimum temperature contrast ' \
                          'of {}K.'.format(dt_min)
                    raise ValueError(msg)
    ...
    def compute_model(self, data_wave_low=None, data_wave_high=None,
        ...
        # Combine photosphere with heterogeneities to make full model of
        # stellar surface.
...
class StellarGrid:
    """Class to create and store a grid of stellar models across a specified
    range of temperature and log gravity values. Currently, only solar
    metallicity models are considered.
    """

    def __init__(self, temperatures, log_gs, input_dir, flux_conv_factor,
                 model_type='PHOENIX', silent=False):
        ...
        flux_conv_factor : float
            Factor converting from flux at the stellar surface to flux
            received at Earth, i.e., (radius_star / distance_from_Earth)^2.
        model_type : str
            Model grid identifier, either 'PHOENIX' or 'SPHINX'
        silent : bool
    ...
    def load_grid(self, wave_low=0.5, wave_high=3, prebin_res=10000,
        ...
        if self.model_type == 'PHOENIX':
            waves, stellar_grid = load_phoenix_grid(self.temperatures,
                                                    self.log_gs,
                                                    self.input_dir,
                                                    self.flux_conv_factor,
                                                    silent=self.silent,
                                                    wave_low=wave_low,
                                                    wave_high=wave_high,
...
def load_phoenix_grid(temperatures, log_gs, input_dir, flux_conv_factor,
    ...
    input_dir : str
        Path to directory containing the stellar models.
    flux_conv_factor : float
        Factor converting from flux at the stellar surface to flux received at
        Earth, i.e., (radius_star / distance_from_Earth)^2.
    wave_low : float
        Low wavelength cut off for stellar mdoels (in µm).
    wave_high : float
        High wavelength cut off for stellar mdoels (in µm).
    ...
    # Ensure that temperatures and gravities are within the allowed range for
    # the grid.
    if np.max(temperatures) > 7000 or np.min(temperatures) < 2300:
        raise ValueError('Temperatures for the PHOENIX grid must be between '
                         '2300K and 7000K.')
    if np.max(log_gs) > 5.5 or np.min(log_gs) < 1.0:
        raise ValueError('log g values for the PHOENIX grid must be between '
                         '1.0 and 5.5.')

    fancyprint('Loading PHOENIX model grid for temperatures in range {0} -- '
               '{1}K and log gravity in range {2} -- {3}.'
               ''.format(temperatures[0], temperatures[1], log_gs[0], log_gs[1]))
    ...
    for i in tqdm(range(len(t_array))):
        ...
        if first_time is True:
            ...
            w = wave_low
            # Bin model wavelengths to specified pre-binned resoltion to speed
            # up computations.
            prebin_waves = []
            while w <= wave_high:
...
def load_sphinx_grid(temperatures, log_gs, input_dir, flux_conv_factor,
                     wave_low, wave_high):
    """Load a grid of SPHINX stellar models. No pre-binning occurs due to the
    low resolution of public SPHINX models.
    ...
    # SPHINX Grid
    temperatures, log_gs = np.array(temperatures), np.array(log_gs)
    # Ensure that the temperatures and gravities don't exceed the allowed
    # bounds for the grid.
    if np.max(temperatures) > 4000 or np.min(temperatures) < 2000:
        raise ValueError('Temperatures for the SPHINX grid must be between '
                         '2000K and 4000K.')
    if np.max(log_gs) > 5.5 or np.min(log_gs) < 4.0:
        raise ValueError('log g values for the SPHINX grid must be between '
                         '4.0 and 5.5.')

    fancyprint('Loading SPHINX model grid for temperatures in range {0} -- '
               '{1}K and log gravity in range {2} -- {3}.'
               ''.format(temperatures[0], temperatures[1], log_gs[0], log_gs[1]))
